...
## gff stat summary
for i in $(cat gff3_list); do echo -n "${i}"; gene_num=$(grep -v "#" ${i} | cut -f3 | sort | uniq -c | grep " gene"); mRNA_num=$(grep -v "#" ${i} | cut -f3 | sort | uniq -c | grep " mRNA"); echo -n "${mRNA_num} "; echo "${gene_num}" ; done

## mapping ration summary
